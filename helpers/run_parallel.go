...
func TestParallel(t *testing.T, workers, noIterations int, fn func() error, allowedErrors ...error) {
	t.Helper()
...
func BenchmarkParallel(b *testing.B, workers, noIterations int, fn func() error, allowedErrors ...error) {
	b.Helper()
		...
		wg.Go(func() {
			for {
				...
				if errRun := fn(); errRun != nil {
					...
					// Atomic swap ensures we only log the FIRST error that kills the run
					if !stop.Swap(true) {
						b.Errorf("parallel worker error: %v", errRun)
					}
